...
assert(
  ...
  quantile_cens(x=1:10, probs=0.3, cens="left", limit=0) ==
    quantile(x=1:10, probs=0.3)
)
assert(
  "quantile_cens returns NA_real_ when the probs value is censored",
  quantile_cens(x=1:10, probs=0.3, cens="left", limit=6) %==%
    NA_real_
)
assert(
  "quantile_cens returns NA_real_ when the probs value is censored",
  quantile_cens(x=1:10, probs=0.3, cens="right", limit=6) %==%
...
assert(
  ...
  quantile_cens(x=c(1:10, rep(NA, 10)), probs=0.3, cens="right", limit=6) %==%
    NA_real_
)
